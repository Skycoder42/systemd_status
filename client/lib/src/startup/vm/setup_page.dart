...
class SetupPage extends ConsumerStatefulWidget {
  ...
  const SetupPage({super.key, required this.setupCompleter});
...
class _SetupPageState extends ConsumerState<SetupPage> {
  ...
  Widget build(BuildContext context) => Scaffold(
    appBar: AppBar(title: Text(context.strings.setup_page_title)),
    body: Form(
      key: _formKey,
            ...
            child: Column(
              ...
              children: [
                ...
                TextFormField(
                  ...
                  readOnly: true,
                  decoration: InputDecoration(
                    label: Text(context.strings.setup_page_server_cert_label),
                    suffix: IconButton(
  ...
  Future<void> _submit() async {
    ...
    try {
      if (_savedCertBytes case final Uint8List certBytes) {
        dio.httpClientAdapter = const HttpClientAdapterFactory().create(
          certBytes,
        );
      }
    ...
    if (!widget.setupCompleter.isCompleted) {
      widget.setupCompleter.complete(
        SetupResult(serverUrl: _savedUrl!, serverCertBytes: _savedCertBytes),
      );
